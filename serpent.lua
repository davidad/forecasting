...
local function s(t, opts)
  ...
  local alphanumsort = type(opts.sortkeys) == 'function' and opts.sortkeys or function(o, n)
    local maxn, to = tonumber(n) or 12, {number = 'c', string = 'b'}
    local function padnum(d) return ("%0"..maxn.."d"):format(d) end
    table.sort(o, function(a,b)
      return (to[type(a)] or 'z')..(tostring(a):gsub("%d+",padnum))
           < (to[type(b)] or 'z')..(tostring(b):gsub("%d+",padnum)) end) end
  local function val2str(t, name, indent, path, plainindex, level)
    ...
    elseif ttype == "table" then
      ...
      for n, key in ipairs(o) do
        ...
        or opts.ignore_keys and opts.ignore_keys[key] -- skip ignored keys
